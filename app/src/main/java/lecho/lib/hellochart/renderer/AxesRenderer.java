package lecho.lib.hellochart.renderer;
...
import lecho.lib.hellochart.util.FloatUtils;
import lecho.lib.hellochart.view.Chart;
...
public class AxesRenderer {
    ...
    //设置坐标轴偏移量
    public void setOffsetLines(int offsetLines) {
    ...
    private void drawAxisLines(Canvas canvas, Axis axis, int position) {
        ...
        if (axis.hasLines()) {
            ...
            for (; valueToDrawIndex < valuesToDrawNumTab[position]; ++valueToDrawIndex) {
                if (isAxisVertical) {
                    if(valueToDrawIndex == DEFAULT_OFFSET_LINES) continue;
                    lineY1 = lineY2 = rawValuesTab[position][valueToDrawIndex];
                } else {

//                    lineX1 = lineX2 = rawValuesTab[position][(valuesToDrawNumTab[position] - valueToDrawIndex -1)];
...
//            linePaintTab[0].setPathEffect(new DashPathEffect(new float[]{5,5,5,5},1));
    ...
    private void drawAxisLabelsAndName(Canvas canvas, Axis axis, int position) {
        ...
        for (int valueToDrawIndex = 0; valueToDrawIndex < valuesToDrawNumTab[position]; ++valueToDrawIndex) {
            System.out.println("valuesToDrawNumTab[position]:" + valuesToDrawNumTab[position]);
            int charsNumber = 0;
            if (axis.isAutoGenerated()) {
